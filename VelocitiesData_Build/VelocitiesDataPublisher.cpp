...
/************************************************************************
 * LOGICAL_NAME:    VelocitiesDataPublisher.cpp
 ...
 * Description:
 *
 * This file contains the implementation for the 'VelocitiesDataPublisher'
...
#include "ccpp_VelocitiesData.h"  /* Include the generated type specific (VelocitiesData) DCPS API */
...
{
  OS_API_EXPORT
  int VelocitiesDataPublisher(int argc, char *argv[]);
}

/* Main wrapper to allow embedded usage of the Publisher application. */
int OSPL_MAIN (int argc, char *argv[])
{
  return VelocitiesDataPublisher(argc, argv);
}
...
int VelocitiesDataPublisher (int argc, char *argv[])
{
    ...
    /* The Application VelocitiesData Data TypeSupport */
    VelocitiesData::VelocitiesTypeSupport_var typesupport;
  ...
  // Register the application data type
    typesupport = new VelocitiesData::VelocitiesTypeSupport();
  ...
  // Create Topic entity
    ...
    /* Use the changed policy when defining the VelocitiesData topic */
    topic = participant->create_topic("VelocitiesData_Velocities", typeName, tQos, NULL, DDS::STATUS_MASK_NONE);
  ...
  // Create Publisher entity
    ...
    pQos.partition.name[0] = "VelocitiesData Partition";
  ...
  // create DataWriter entity
    ...
    /* Cast writer to 'VelocitiesData' type specific interface. */
    VelocitiesData::VelocitiesDataWriter_var VelocitiesDataWriter = VelocitiesData::VelocitiesDataWriter::_narrow(writer);
    checkHandle(VelocitiesDataWriter, "VelocitiesDataWriter::_narrow() failed");

   // for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
int count = 0;
 for (count=0;; ++count) {
  // Create and Publish the HelloWorld message
    VelocitiesData::Velocities msg; /* declare a Message on Stack */
    //msg.userID = count;
    ...
    result = VelocitiesDataWriter->write(msg, DDS::HANDLE_NIL);
