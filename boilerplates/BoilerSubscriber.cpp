...
{
  OS_API_EXPORT
  int BoilerSubscriber(int argc, char *argv[]);
}

/* Main wrapper to allow embedded usage of the Subscriber application. */
int OSPL_MAIN (int argc, char *argv[])
{
  return BoilerSubscriber (argc, argv);
}
...
int BoilerSubscriber(int argc, char *argv[])
{
    ...
    Boiler::BOILER_STRUCT_TYPE_Seq msgList;
  ...
  // Create Subscriber entity
    ...
    sQos.partition.name[0] = "Boiler Partition";
  ...
  // create DataReader entity
    ...
    Boiler::BOILER_STRUCT_TYPE_DataReader_var BoilerReader = Boiler::BOILER_STRUCT_TYPE_DataReader::_narrow(reader);
    checkHandle(BoilerReader, "BOILER_STRUCT_TYPE_DataReader::_narrow() failed");
...
while (true)
    {
        result = BoilerReader->take(msgList, infoSeq, DDS::LENGTH_UNLIMITED,
        ...
        result = BoilerReader->return_loan(msgList, infoSeq);
        checkStatus(result, "BOILER_STRUCT_TYPE_DataReader::return_loan() failed");
