...
/************************************************************************
 * LOGICAL_NAME:    BoilerPublisher.cpp
 ...
 * Description:
 *
 * This file contains the implementation for the 'BoilerPublisher'
...
#include "ccpp_Boiler.h"  /* Include the generated type specific (Boiler) DCPS API */
...
int BoilerPublisher (int argc, char *argv[])
{
    ...
    /* The Application Boiler Data TypeSupport */
    Boiler::BOILER_STRUCT_TYPE_TypeSupport_var typesupport;
  ...
  // Register the application data type
    typesupport = new Boiler::BOILER_STRUCT_TYPE_TypeSupport();
  ...
  // Create Topic entity
    ...
    /* Use the changed policy when defining the Boiler topic */
    topic = participant->create_topic("Boiler_BOILER_STRUCT_TYPE_", typeName, tQos, NULL, DDS::STATUS_MASK_NONE);
  ...
  // Create Publisher entity
    ...
    pQos.partition.name[0] = "Boiler Partition";
  ...
  // create DataWriter entity
    ...
    /* Cast writer to 'Boiler' type specific interface. */
    Boiler::BOILER_STRUCT_TYPE_DataWriter_var BoilerWriter = Boiler::BOILER_STRUCT_TYPE_DataWriter::_narrow(writer);
    checkHandle(BoilerWriter, "BOILER_STRUCT_TYPE_DataWriter::_narrow() failed");

   // for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
int count = 0;
 for (count=0;; ++count) {
  // Create and Publish the HelloWorld message
    Boiler::BOILER_STRUCT_TYPE_ msg; /* declare a Message on Stack */
    //msg.userID = count;
    ...
    result = BoilerWriter->write(msg, DDS::HANDLE_NIL);
